...
class MotionSmoothLayer(torch.nn.Module):
    def __init__(self, exp_name = "", penalty_weight=1.0):
        super(MotionSmoothLayer, self).__init__()
        # default configs
    ...
    def forward(self, video_seq):
        ...
        norm_attention = (smoothed_outers - torch.min(smoothed_outers)) / (torch.max(smoothed_outers) - torch.min(smoothed_outers) + 1e-6)
        norm_attention = norm_attention.unsqueeze(2)
